<template>
  <section>
    ...
    <div class="min-h-[110px] flex flex-row items-center justify-center gap-2">
      <div
        v-for="(pc, i) in sorted"
        :key="pc.carte?.id ?? `${pc.ordre}-${i}`"
...
function teamOfSeat(seat: number) {
  return seat % 2 === 0 ? 1 : 2;
}
...
.ring-team1 {
  box-shadow: 0 0 0 4px #2563eb inset, 0 2px 6px rgba(0, 0, 0, 0.35);
}
.ring-team2 {
  box-shadow: 0 0 0 4px #f59e0b inset, 0 2px 6px rgba(0, 0, 0, 0.35);
}
</style>
