  ...
  <div class="modal-backdrop grid place-items-center p-6">
    <div class="card p-6 max-w-3xl w-full">
      <header class="flex items-center justify-between mb-4">
        ...
        <button class="px-3 py-1 rounded bg-slate-800" @click="goBack">Fermer</button>
      </header>

      <div v-if="store.loading">Chargement…</div>
      <div v-else-if="store.error" class="text-red-400">Erreur: {{ store.error }}</div>

      <div v-else-if="store.data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
...
import { useStatsStore } from '@/stores/stats'                 // ✅ store API
