...
const S = (over?: Partial<Seat>): Seat => ({
  seat: 0,
  joueurId: 1,
  team: 1,
  ...over,
});
const card = (id: number, valeur: string, couleurId: number): C => ({
  id,
  valeur,
  couleurId,
});
...
describe('RulesService', () => {
  ...
  it('obligation de fournir la couleur demandée si on l’a', () => {
    ...
    const trick: TrickCard[] = [
      { ordre: 0, joueurId: 99, carte: card(100, '7', 1) },
    ]; // couleur demandée = 1
    const playable = rules.playableCards({
      hand,
      trickCards: trick,
  ...
  it('beats() : à l’atout Valet > 9 > As… et hors atout As > 10 > Roi…', () => {
    ...
    // à l’atout
    expect(
      rules.beats(card(1, 'Valet', 4), card(2, '9', 4), atoutId, demande),
    ).toBe(true);
    expect(
      rules.beats(card(3, '9', 4), card(4, 'Valet', 4), atoutId, demande),
    ).toBe(false);
    // hors atout (couleur demandée)
    expect(
      rules.beats(card(5, 'As', 1), card(6, '10', 1), atoutId, demande),
    ).toBe(true);
    expect(
      rules.beats(card(7, '10', 1), card(8, 'As', 1), atoutId, demande),
    ).toBe(false);
    // autre couleur ne bat pas la demandée (hors atout)
    expect(
      rules.beats(card(9, 'As', 2), card(10, '7', 1), atoutId, demande),
    ).toBe(false);
  });
  ...
  it('isPlayable() reflète playableCards()', () => {
    ...
    const trick: TrickCard[] = [
      { ordre: 0, joueurId: 9, carte: card(100, '7', 1) },
    ];
