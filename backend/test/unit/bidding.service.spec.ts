...
const baseManche = (over?: Partial<any>) => ({
  ...
  partie: {
    statut: 'en_cours',
    equipes: [
      { joueurs: [{ ordreSiege: 0, joueurId: 10 }, { ordreSiege: 2, joueurId: 12 }] },
      { joueurs: [{ ordreSiege: 1, joueurId: 11 }, { ordreSiege: 3, joueurId: 13 }] },
    ],
    lobby: null
  },
...
describe('BiddingService', () => {
  ...
  it('tour 1 : CHOOSE_COLOR interdit → BadRequest', async () => {
    prisma._tx.manche.findUnique.mockResolvedValueOnce(baseManche({ tourActuel: 1, joueurActuelId: 10 }));
    prisma._tx.manche.findFirst.mockResolvedValueOnce({ id: 1, numero: 1 }); // latest = self

    await expect(service.placeBid(1, {
      joueurId: 10, type: 'choose_color' as any, couleurAtoutId: 2
  ...
  it('tour 2 : TAKE_CARD interdit → BadRequest', async () => {
    prisma._tx.manche.findUnique.mockResolvedValueOnce(baseManche({ tourActuel: 2, joueurActuelId: 10 }));
    prisma._tx.manche.findFirst.mockResolvedValueOnce({ id: 1, numero: 1 });

    await expect(service.placeBid(1, {
      joueurId: 10, type: 'take_card' as any
    })).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('manche introuvable → NotFound', async () => {
    ...
    await expect(service.placeBid(1, { joueurId: 10, type: 'pass' as any }))
      .rejects.toBeInstanceOf(NotFoundException);
  });

  it('ordre de tour : si joueurActuel ≠ joueurId → BadRequest', async () => {
    prisma._tx.manche.findUnique.mockResolvedValueOnce(baseManche({ joueurActuelId: 11 }));
    prisma._tx.manche.findFirst.mockResolvedValueOnce({ id: 1, numero: 1 });
