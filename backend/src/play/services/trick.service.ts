...
export class TrickService {
  ...
  async closeCurrentTrick(mancheId: number) {
    ...
    const result = await this.prisma.$transaction(
      async (tx) => {
        ...
        const plis = [...(manche.plis || [])].sort(
          (a, b) => a.numero - b.numero,
        );
        if (plis.length === 0)
          throw new BadRequestException(
            `Aucun pli en cours pour cette manche.`,
          );
        const last = plis[plis.length - 1];
        if (last.cartes.length !== 4) {
          throw new BadRequestException(
            `Le pli ${last.numero} n'est pas complet (${last.cartes.length}/4).`,
          );
        }
        ...
        const { team1, team2, winnerTeam } = await this.computeLiveScores(
          tx,
          mancheId,
          atoutId,
          winnerId,
    ...
    if (result.requiresEndOfHand) {
      try {
        ...
        return {
          ...result,
          endOfHandError: (e as Error).message ?? 'endOfHand failed',
        };
      }
    }
  ...
  private async computeLiveScores(
    ...
    manche!.partie.equipes.forEach((eq) => {
      eq.joueurs.forEach((j) => {
        mapTeam[j.joueurId] = eq.numero as 1 | 2;
      });
    });
  ...
  async scoreLive(mancheId: number) {
    ...
    const { team1, team2 } = await this.computeLiveScores(
      this.prisma,
      mancheId,
      manche.couleurAtoutId ?? null,
      -1,
    );
