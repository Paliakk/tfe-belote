...
export class RulesService {
  ...
  playableCards(params: {
    ...
    const cartesCouleurDemandee = hand.filter(
      (c) => c.couleurId === couleurDemandee,
    );
    const cartesAtout = atoutId
      ? hand.filter((c) => c.couleurId === atoutId)
      : [];
    ...
    if (cartesAtout.length > 0 && atoutId != null) {
      const atoutsDejaJoues = trickCards
        .filter((tc) => tc.carte.couleurId === atoutId)
        .map((tc) => tc.carte);
      // Exception partenaire maître : si mon partenaire mène le pli, je peux NE PAS couper
      const partnerLeads =
        leaderTeam === currentTeam && winning.joueurId !== currentPlayerId;
      ...
      const surcoupePossible = cartesAtout.some((c) =>
        this.isStrongerAtout(c, plusFortAtout),
      );

      if (surcoupePossible) {
        return cartesAtout
          .filter((c) => this.isStrongerAtout(c, plusFortAtout))
          .map((c) => c.id);
      }
  ...
  currentWinning(
    trickCards: TrickCard[],
    atoutId: number | null,
    couleurDemandee: number,
  ) {
    let winner = trickCards[0];
    for (let i = 1; i < trickCards.length; i++) {
      const challenger = trickCards[i];
      if (
        this.beats(challenger.carte, winner.carte, atoutId, couleurDemandee)
      ) {
        winner = challenger;
      }
    }
  ...
  beats(
    ...
    if (
      atoutId != null &&
      candidate.couleurId === atoutId &&
      current.couleurId !== atoutId
    ) {
      return true;
    }
    ...
    if (
      ...
      return (
        this.strengthAtout[candidate.valeur] >
        this.strengthAtout[current.valeur]
      );
    }
    // Sinon, hors atout : seule la COULEUR DEMANDÉE peut battre la COULEUR DEMANDÉE
    if (
      candidate.couleurId === couleurDemandee &&
      current.couleurId === couleurDemandee
    ) {
      return (
        this.strengthNonAtout[candidate.valeur] >
        this.strengthNonAtout[current.valeur]
      );
    }
