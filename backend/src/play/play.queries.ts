...
export class PlayQueriesService {
  ...
  async getHand(mancheId: number, joueurId: number) {
    ...
    if (!manche)
      return {
        ...
        joueurId,
        cartes: [],
        message: `Manche ${mancheId} introuvable.`,
      };
  ...
  async getPlayable(mancheId: number, joueurId: number) {
    ...
    if (!manche)
      return { playableIds: [], message: `Manche ${mancheId} introuvable.` };
    if (manche.joueurActuelId !== joueurId) {
      return {
        playableIds: [],
        message: `Ce n'est pas au joueur ${joueurId} de jouer.`,
      };
    }
  ...
  async getActiveTrick(mancheId: number) {
    ...
    if (!manche)
      return { mancheId, message: `Manche ${mancheId} introuvable.` };
  ...
  async getActiveManche(partieId: number) {
    ...
    if (!partie || partie.manches.length === 0)
      return { partieId, activeMancheId: null };
