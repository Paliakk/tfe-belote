      ...
      async (tx) => {
        ...
        if (!manche.partie?.equipes || manche.partie.equipes.length !== 2) {
          throw new BadRequestException(
            `Manche ${mancheId}: parties/équipes invalides.`,
          );
        }
        ...
        manche.partie.equipes.forEach((eq) => {
          ...
          eq.joueurs.forEach(
            (j) => (joueurToTeamNumero[j.joueurId] = eq.numero as 1 | 2),
          );
        });

        const atoutId =
          manche.couleurAtout?.id ?? manche.couleurAtoutId ?? null;
        ...
        for (const pli of manche.plis) {
          const sumPli = pli.cartes.reduce((sum, pc) => {
            ...
            return (
              sum + (isAtout ? pc.carte.pointsAtout : pc.carte.pointsNonAtout)
            );
          }, 0);
        ...
        const lastTrickWinnerTeam: 1 | 2 =
          joueurToTeamNumero[manche.plis[7].gagnantId!];
        const bonusDix = 10;
        ...
        const preneurTeamNumero: 1 | 2 | null =
          preneurId != null ? joueurToTeamNumero[preneurId] : null;
        const preneurEquipeId = preneurTeamNumero
          ? teamNumeroToEquipeId.get(preneurTeamNumero)!
          : null;
        ...
        const bonusMap: Record<
          1 | 2,
          { pts: number; details: TeamScore['detailsBonus'] }
        > = {
          1: { pts: 0, details: [] },
          2: { pts: 0, details: [] },
        };
        ...
        bonusMap[lastTrickWinnerTeam].details.push({
          type: 'dix_de_der',
          points: bonusDix,
        });
        // capot
        if (capotTeam) {
          ...
          bonusMap[capotTeam].details.push({
            type: 'capot',
            points: bonusCapot,
          });
        }
        ...
        if (beloteTeam) {
          ...
          bonusMap[beloteTeam].details.push({
            type: 'belote',
            points: bonusBelote,
          });
        }
